-- ============================================================================
-- PROCEDIMIENTO ALMACENADO: PROYECCION DE COMISIONES
-- DESCRIPCIÓN: Proyecta comisiones para vendedores y facturadores
-- PARÁMETROS: @fechaInicio y @fechaFin (ej: '2026-05-01' y '2026-05-31')
-- USO: CALL sp_proyeccion_comisiones('2026-05-01', '2026-05-31');
-- ============================================================================

DELIMITER $$

DROP PROCEDURE IF EXISTS sp_proyeccion_comisiones$$

CREATE PROCEDURE sp_proyeccion_comisiones(
...
BEGIN
    ...
    -- PASO 9: Configuracion mora (tabla vacía)
    DROP TEMPORARY TABLE IF EXISTS tmp_configs_gracia;
    CREATE TEMPORARY TABLE tmp_configs_gracia (rol_id INT, tipo_factura VARCHAR(20), dias_gracia INT, porcentaje_retencion DECIMAL(5,2)) ENGINE=MEMORY;
    ...
    SELECT tfr.*, CASE WHEN tfr.tipo_pago_id = 1 THEN 'contado' ELSE 'credito' END AS tipo_factura_pago, CASE WHEN tfr.tipo_pago_id = 1 THEN DATEDIFF(tfr.fecha_pago_cierre, tfr.fecha_emision) ELSE DATEDIFF(tfr.fecha_pago_cierre, tfr.fecha_vencimiento) END AS dias_transcurridos, COALESCE(cg.dias_gracia, 0) AS dias_gracia, COALESCE(cg.porcentaje_retencion, 0) AS porcentaje_retencion
    FROM tmp_totales_factura_rol tfr
    LEFT JOIN tmp_configs_gracia cg ON cg.rol_id = tfr.rol_id AND cg.tipo_factura = CASE WHEN tfr.tipo_pago_id = 1 THEN 'contado' ELSE 'credito' END;

    -- PASO 11: Detalle final con retencion mora
    DROP TEMPORARY TABLE IF EXISTS tmp_detalle_final;
    CREATE TEMPORARY TABLE tmp_detalle_final AS
    SELECT dmb.*, 
        CASE WHEN dmb.dias_gracia IS NULL OR dmb.dias_gracia <= 0 OR dmb.dias_transcurridos <= 0 THEN 0 WHEN dmb.tipo_pago_id = 1 AND dmb.dias_transcurridos > dmb.dias_gracia THEN 1 WHEN dmb.tipo_pago_id <> 1 AND dmb.dias_transcurridos > dmb.dias_gracia THEN FLOOR(dmb.dias_transcurridos / dmb.dias_gracia) ELSE 0 END AS periodos_vencidos,
        CASE WHEN dmb.dias_gracia IS NULL OR dmb.dias_gracia <= 0 OR dmb.dias_transcurridos <= 0 THEN 0.0000 WHEN dmb.tipo_pago_id = 1 AND dmb.dias_transcurridos > dmb.dias_gracia THEN ROUND(dmb.comision_bruta, 4) WHEN dmb.tipo_pago_id <> 1 AND COALESCE(dmb.porcentaje_retencion, 0) > 0 AND FLOOR(dmb.dias_transcurridos / dmb.dias_gracia) > 0 THEN LEAST(ROUND(dmb.comision_bruta, 4), ROUND(ROUND(dmb.comision_bruta * (dmb.porcentaje_retencion / 100), 4) * FLOOR(dmb.dias_transcurridos / dmb.dias_gracia), 4)) ELSE 0.0000 END AS retencion_mora_proyectada
    ...
    SELECT df.periodo_pago, df.fecha_pago_cierre, df.factura_id, df.aplicacion_pagos_id, df.cai, df.capacidad, df.user_id, df.empleado, df.rol_id, df.rol_nombre, df.tipo_factura_codigo, df.tipo_factura_pago, df.es_sr, df.categorias_comision, ROUND(df.comision_bruta, 4) AS comision_bruta, df.dias_transcurridos, df.dias_gracia, COALESCE(df.porcentaje_retencion, 0) AS porcentaje_retencion_mora, df.periodos_vencidos, ROUND(df.retencion_mora_proyectada, 4) AS retencion_mora_proyectada, ROUND(GREATEST(0, df.comision_bruta - df.retencion_mora_proyectada), 4) AS comision_neta_mora
    FROM tmp_detalle_final df;

    -- ============================================================================
    -- RESULTADO 1: RESUMEN POR EMPLEADO
    -- ============================================================================
    SELECT 'RESULTADO_1_RESUMEN_POR_EMPLEADO' AS resultado_tipo;
    ...
    -- ============================================================================
    SELECT 'RESULTADO_2_DETALLE_FACTURA' AS resultado_tipo;
    SELECT periodo_pago, fecha_pago_cierre, factura_id, aplicacion_pagos_id, cai, capacidad, user_id, empleado, rol_id, rol_nombre, tipo_factura_codigo, tipo_factura_pago, es_sr, categorias_comision, comision_bruta, dias_transcurridos, dias_gracia, porcentaje_retencion_mora, periodos_vencidos, retencion_mora_proyectada, comision_neta_mora
    FROM tmp_proyeccion_comisiones_vf
    ORDER BY fecha_pago_cierre, cai, capacidad, empleado
    LIMIT 100;

    -- ============================================================================
    -- RESULTADO 3: TOTALES CONSOLIDADOS
    -- ============================================================================
    SELECT 'RESULTADO_3_TOTALES' AS resultado_tipo;
    SELECT COUNT(DISTINCT factura_id) AS total_facturas, ROUND(SUM(comision_bruta), 4) AS comision_bruta_total, ROUND(SUM(comision_neta_mora), 4) AS comision_neta_total, COUNT(DISTINCT user_id) AS total_empleados FROM tmp_proyeccion_comisiones_vf;

    -- ============================================================================
    -- RESULTADO 4: RESUMEN POR TIPO (SR vs NORMAL)
    -- ============================================================================
    SELECT 'RESULTADO_4_POR_TIPO_SR' AS resultado_tipo;
    SELECT CASE WHEN es_sr = 1 THEN 'SR' ELSE 'NORMAL' END AS tipo_regla, COUNT(DISTINCT factura_id) AS facturas, ROUND(SUM(comision_bruta), 4) AS comision_bruta, ROUND(SUM(comision_neta_mora), 4) AS comision_neta
    FROM tmp_proyeccion_comisiones_vf
    GROUP BY CASE WHEN es_sr = 1 THEN 'SR' ELSE 'NORMAL' END
    ORDER BY tipo_regla;
...
-- ============================================================================
-- CALL sp_proyeccion_comisiones('2026-05-01', '2026-05-31');
-- CALL sp_proyeccion_comisiones('2026-01-01', '2026-12-31');
-- CALL sp_proyeccion_comisiones('2026-06-01', '2026-06-30');
-- ============================================================================
