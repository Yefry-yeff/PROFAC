...
class ReporteClientesHoja2 implements FromArray, WithTitle, WithStyles, WithDrawings, WithEvents
{
    protected $rows;
    protected $usuario;
    const LAST_COL = 'F';
    ...
    public function title(): string { return 'Sin Crédito'; }
    ...
    public function array(): array
    {
        ...
        // Row 2: report title
        $out[] = ['REPORTE DE CLIENTES SIN CRÉDITO', '', '', '', '', ''];

        // Row 3: date + user
        $out[] = ['Generado: ' . now()->format('d/m/Y H:i') . '   |   Descargado por: ' . $this->usuario, '', '', '', '', ''];

        // Row 4: column headers
        $out[] = ['#', 'VENDEDOR', 'CLIENTE', 'CÓDIGO', 'ESTADO', 'OBSERVACIONES'];
