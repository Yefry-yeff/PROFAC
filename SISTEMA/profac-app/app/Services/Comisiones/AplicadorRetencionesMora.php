...
/**
 * Evalúa si corresponde aplicar retención por mora sobre comisiones
 * ya generadas por GeneradorFacturasComision.
 *
 * Se debe invocar DESPUÉS de generar() y ANTES de procesar():
 *
 *   $array = $generador->generar($facturaId, $apId, $fecha);
 *   $array = $retencion->aplicar($array, $facturaId, $fechaCierre);
 *   foreach ($array as $fc) { $procesador->procesar($fc); }
 *
 * REGLAS:
 *  - Contado: referencia = fecha_emision.  diasTranscurridos = fechaCierre - fecha_emision
 *  - Crédito: referencia = fecha_vencimiento. diasTranscurridos = fechaCierre - fecha_vencimiento
 *  - Si diasTranscurridos > diasGracia Y porcentaje_retencion > 0 → aplica retención.
 *  - montoRetencion = sub_total_factura * (porcentaje_retencion / 100)
 *  - monto_final    = max(0, monto_rol - montoRetencion)
 *  - Se actualiza facturas_comision.monto_rol y se registran los campos de auditoría.
 *  - Una retención solo se aplica una vez (guardia: retencion_mora_monto NOT NULL).
 */
class AplicadorRetencionesMora
{
    /**
     * @param  array       $comisionesGeneradas  Array devuelto por GeneradorFacturasComision::generar()
     * @param  int         $facturaId
     * @param  string|null $fechaCierre          YYYY-MM-DD. Si null, se usa hoy.
     * @return array                             El mismo array con monto_rol ajustado donde aplica.
     */
    public function aplicar(array $comisionesGeneradas, int $facturaId, ?string $fechaCierre = null): array
    {
        if (empty($comisionesGeneradas)) {
        ...
        // Datos de la factura: tipo de pago, fechas de referencia y subtotal
        $factura = DB::table('factura')
            ->where('id', $facturaId)
            ->select('tipo_pago_id', 'fecha_emision', 'fecha_vencimiento', 'sub_total')
            ->first();
        ...
        // 1 = contado, 2 = crédito (tipo_pago_venta)
        $esContado = (int) $factura->tipo_pago_id === 1;
        $tipoKey   = $esContado ? 'contado' : 'credito';

        // Fecha de referencia según tipo
        $fechaReferencia = $esContado
            ? Carbon::parse($factura->fecha_emision)->startOfDay()
            : Carbon::parse($factura->fecha_vencimiento)->startOfDay();

        $diasTranscurridos = (int) $fechaReferencia->diffInDays($fechaCierreCarbon, false);
        // diffInDays(false) → negativo si fechaCierre < fechaRef (pago anticipado)
        // Solo interesa el caso tardío (positivo)

        $subTotal = (float) $factura->sub_total;

        // Cargar configuración de días de gracia para todos los roles involucrados en una sola query
        $rolIds = array_unique(array_column($comisionesGeneradas, 'rol_id'));

        $configs = DB::table('dias_gracia_comision')
            ->whereIn('rol_id', $rolIds)
            ->where('tipo_factura', $tipoKey)
            ->where('dias_gracia', '>', 0)       // Solo configs con días definidos
            ->where('porcentaje_retencion', '>', 0) // Solo si hay porcentaje configurado
            ->get()
            ->keyBy('rol_id');
        ...
        foreach ($comisionesGeneradas as &$fc) {
            ...
            // Guardia de idempotencia: si ya tiene retención aplicada, omitir
            $yaAplicada = DB::table('facturas_comision')
                ->where('id', $fcId)
                ->whereNotNull('retencion_mora_monto')
                ->exists();
            ...
            if ($diasTranscurridos <= $diasGracia) {
                continue; // Dentro del período de gracia, no hay retención
            }

            $porcentaje      = (float) $config->porcentaje_retencion;
            $montoRetencion  = round($subTotal * ($porcentaje / 100), 4);
            $montoOriginal   = (float) $fc['monto_rol'];
            $montoFinal      = max(0.0, $montoOriginal - $montoRetencion);

            // Actualizar facturas_comision con el monto ajustado y datos de auditoría
            DB::table('facturas_comision')
                ->where('id', $fcId)
                ->update([
                    'monto_rol'             => round($montoFinal, 4),
                    'retencion_mora_monto'  => round($montoRetencion, 4),
                    'retencion_mora_dias'   => $diasTranscurridos,
                    'updated_at'            => now(),
                ]);

            // Propagar el monto ajustado al array para que ProcesadorComisiones
            // acredite el valor correcto a comision_empleado
            $fc['monto_rol'] = round($montoFinal, 4);
        }
        unset($fc);
