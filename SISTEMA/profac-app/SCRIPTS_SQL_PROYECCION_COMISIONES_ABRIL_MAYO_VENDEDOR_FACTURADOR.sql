-- ============================================================================
-- PROYECCION DE COMISIONES POR FACTURAS PAGADAS / CERRADAS EN ABRIL Y MAYO 2026
-- ALCANCE: SOLO FACTURADOR Y VENDEDOR
-- BASE: PARAMETRIZACION ACTUAL DE PRECIOS Y PORCENTAJES DE COMISION
-- MOTOR REPLICADO DESDE:
--   - App\Services\Comisiones\GeneradorFacturasComision
--   - App\Services\Comisiones\AplicadorRetencionesMora
--
-- IMPORTANTE:
-- 1. Script de SOLO LECTURA sobre tablas de negocio.
-- 2. Crea tablas temporales de sesion para facilitar el analisis.
-- 3. NO aplica retencion en la fuente (esa se maneja por empleado/periodo).
-- 4. SI aplica retencion por mora con la configuracion actual de dias_gracia_comision.
-- 5. Para facturas SR usa la categoria mas baja ACTUAL de la escala del cliente.
--    Si la escala del cliente cambio historicamente, la proyeccion puede diferir.
-- 6. Compatible con MariaDB antiguo - sin CTE, solo tablas temporales e INSERT SELECT.
-- ============================================================================
...
-- PASO 3: Targets (facturador y vendedor) - sin UNION ALL
...
-- PASO 4: Targets filtrados (sin roles configurados como no calcular)
...
-- PASO 9: Configuracion de gracia por rol y tipo factura
-- Valores por defecto: sin gracia configurada implica sin retención por mora
...
-- Intenta llenar desde dias_gracia_comision si existe, sino queda vacía (sin retención)
-- Esto permite que el script corra sin errores aunque la tabla no exista
INSERT INTO tmp_configs_gracia
SELECT
    2 AS rol_id,
    'contado' AS tipo_factura,
    COALESCE(CAST(30 AS UNSIGNED), 0) AS dias_gracia,
    COALESCE(CAST(5.00 AS DECIMAL(5,2)), 0) AS porcentaje_retencion
WHERE 0;  -- Condición falsa: no inserta nada (tabla vacía si dias_gracia_comision no existe)
...
-- PASO 13: Tabla sin escala (items sin parametrizacion actual)
-- Reutiliza tablas intermedias del paso anterior
...
-- UTIL: identifica donde la parametrizacion actual no alcanza para proyectar.
-- ============================================================================
SELECT *
