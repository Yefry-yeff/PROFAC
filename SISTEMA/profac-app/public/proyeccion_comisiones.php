...
FROM tmp_totales_factura_rol tfr;
        ...
        if (!$conn->multi_query($sql)) {
            throw new Exception('Error en setup: ' . $conn->error);
        }
        
        // Consume todos los resultados del setup
        while ($conn->next_result()) {
            $conn->store_result();
        }

        echo '<div class="status success">✓ Tablas temporales creadas exitosamente</div>';

        // RESULTADO 1
        echo '<div class="resultado"><h2>📋 RESULTADO 1: Resumen por Empleado</h2>';
        $result = $conn->query("
        ...
        if ($result && $result->num_rows > 0) {
            echo '<table><tr><th>Período</th><th>Capacidad</th><th>Empleado</th><th>Rol</th><th>Facturas</th><th>Comisión Bruta</th><th>Retención</th><th>Comisión Neta</th></tr>';
            while ($row = $result->fetch_assoc()) {
                echo '<tr><td>' . $row['periodo_pago'] . '</td><td>' . $row['capacidad'] . '</td><td>' . $row['empleado'] . '</td><td>' . $row['rol_nombre'] . '</td><td>' . $row['facturas_proyectadas'] . '</td><td>$' . $row['comision_bruta_total'] . '</td><td>$' . $row['retencion_mora_total'] . '</td><td>$' . $row['comision_neta_mora_total'] . '</td></tr>';
            }
            echo '</table>';
        } else {
            echo '<p class="error">Sin datos para mayo 2026</p>';
        }
        echo '</div>';

        // RESULTADO 2
        echo '<div class="resultado"><h2>📑 RESULTADO 2: Detalle por Factura (primeras 20)</h2>';
        $result = $conn->query("
        ...
        if ($result && $result->num_rows > 0) {
            echo '<table><tr><th>CAI</th><th>Factura</th><th>Capacidad</th><th>Empleado</th><th>Rol</th><th>Comisión Bruta</th><th>Comisión Neta</th></tr>';
            while ($row = $result->fetch_assoc()) {
                echo '<tr><td>' . $row['cai'] . '</td><td>' . $row['factura_id'] . '</td><td>' . $row['capacidad'] . '</td><td>' . $row['empleado'] . '</td><td>' . $row['rol_nombre'] . '</td><td>$' . $row['comision_bruta'] . '</td><td>$' . $row['comision_neta'] . '</td></tr>';
            }
            echo '</table>';
        } else {
            echo '<p class="error">Sin datos para mayo 2026</p>';
        ...
        // RESULTADO 3
        echo '<div class="resultado"><h2>💰 RESULTADO 3: Totales</h2>';
        $result = $conn->query("
        ...
        if ($result && $row = $result->fetch_assoc()) {
            echo '<table><tr><th>Total Facturas</th><th>Comisión Total</th></tr>';
            echo '<tr><td>' . $row['total_facturas'] . '</td><td>$' . $row['comision_total'] . '</td></tr>';
            echo '</table>';
        }
        echo '</div>';

        echo '<div class="status success">✓ PROYECCIÓN COMPLETADA EXITOSAMENTE</div>';
        $conn->close();
