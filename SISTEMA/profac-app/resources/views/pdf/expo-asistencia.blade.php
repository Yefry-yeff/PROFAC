...
<body>
    ...
    <table>
        <thead><tr><th>ID</th><th>Cliente</th><th>RTN</th><th>Teléfono</th><th>Correo</th><th>Fecha de asistencia</th><th>Tickets</th><th>Regalo</th><th>Comentario</th><th>Usuario</th></tr></thead>
        <tbody>
            @forelse($asistentes as $cliente)
                <tr><td>{{ $cliente->id }}</td><td>{{ $cliente->nombre }}</td><td>{{ $cliente->rtn }}</td><td>{{ $cliente->telefono_empresa }}</td><td>{{ $cliente->correo }}</td><td>{{ date('d/m/Y H:i', strtotime($cliente->registrado_at)) }}</td><td>{{ $cliente->tickets }}</td><td>{{ $cliente->recibio_regalo ? 'Sí' : 'No' }}</td><td>{{ $cliente->comentario ?: '-' }}</td><td>{{ $cliente->registrado_por }}</td></tr>
            @empty
