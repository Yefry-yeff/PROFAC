                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-md-6 col-lg-6 col-xl-6">
                                    <div class="form-group">
                                        ...
                                        <label for="porDescuento" class="col-form-label focus-label">Descuento aplicado %:<span class="text-danger">*</span></label>
                                        <input class="form-control" type="number" min="0" max="50" step="0.01"
                                ...
                                <div class="col-12 col-md-6 col-lg-6 col-xl-6">
                                    ...
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        ...
                                        <div class="input-group">
                                            <input type="text" id="codigoProductoSinRestGobierno" class="form-control"
                                                   placeholder="ID o nombre del producto…" autocomplete="off"
                                                   onkeydown="if(event.key==='Enter'){buscarPorCodigoSinRestGobierno(this.value);return false;}">
                                            <div class="input-group-append">
                                                <button type="button" class="btn btn-primary" title="Buscar producto"
                                                        onclick="limpiarProductoSinRestGobierno(); window['abrirBuscador_buscadorProductoSinRestGobierno'](document.getElementById('codigoProductoSinRestGobierno').value||'')">
                                                    <i class="fa fa-search"></i>
                                                </button>
                                            </div>
                                        </div>
                                        <small id="productoSeleccionadoSinRestGobierno" class="text-success font-weight-bold mt-1 d-block d-none"></small>
                                        ...
                                        <x-buscador-producto id-modal="buscadorProductoSinRestGobierno" callback="alSeleccionarProductoSinRestGobierno" />
                                        @push('scripts')
                                        <script>
                                        function limpiarProductoSinRestGobierno() {
                                            document.getElementById('seleccionarProducto').innerHTML = '<option value="" selected disabled></option>';
                                            document.getElementById('codigoProductoSinRestGobierno').value = '';
                                            var lbl = document.getElementById('productoSeleccionadoSinRestGobierno');
                                            lbl.classList.add('d-none'); lbl.textContent = '';
                                            document.getElementById('historialPreciosPanel').classList.add('d-none');
                                        }
                                        function alSeleccionarProductoSinRestGobierno(producto) {
                                            ...
                                            document.getElementById('codigoProductoSinRestGobierno').value = producto.nombre;
                                            var label = document.getElementById('productoSeleccionadoSinRestGobierno');
                                            label.textContent = '✓ ' + producto.nombre + ' (ID: ' + producto.id + ')';
                                            label.classList.remove('d-none');
                                            cargarCategoriasProducto();
                                            cargarHistorialPrecosSinRestGobierno();
                                        }
                                        function buscarPorCodigoSinRestGobierno(cod) {
                                            cod = String(cod).trim();
                                            if (!cod) { window['abrirBuscador_buscadorProductoSinRestGobierno'](''); return; }
                                            axios.get('/productos/buscar', { params: { q: cod, page: 1 } })
                                                .then(function(r) {
                                                    ...
                                                    if (exact) { alSeleccionarProductoSinRestGobierno(exact); }
                                                    else if (items.length === 1) { alSeleccionarProductoSinRestGobierno(items[0]); }
                                                    else { window['abrirBuscador_buscadorProductoSinRestGobierno'](cod); }
                                                });
                                        }
                                        function cargarHistorialPrecosSinRestGobierno() {
    ...
    @push('scripts')
        ...
        <script>
            ...
            function obtenerImagenes() {
                ...
                // No habilitar bodega automáticamente - se habilita al seleccionar categoría
                ...
                axios.post('/ventas/estatal/guardar', formDataObj, options)
                    .then(response => {
                        ...
                        document.getElementById('seleccionarProducto').innerHTML =
                            '<option value="" selected disabled></option>';
                        document.getElementById('codigoProductoSinRestGobierno').value = '';
                        var lbl_p = document.getElementById('productoSeleccionadoSinRestGobierno');
