    ...
    <div class="wrapper wrapper-content animated fadeInRight">
        ...
        <section class="fa-filters" aria-label="Filtros del dashboard">
            <div class="fa-filter-grid">
                <div class="fa-field"><label>Desde</label><input type="date" class="form-control" wire:model.live="fechaDesde"></div>
                <div class="fa-field"><label>Hasta</label><input type="date" class="form-control" wire:model.live="fechaHasta"></div>
                <div class="fa-field"><label>Cliente / RTN</label><input type="text" class="form-control" wire:model.live="filtroCliente" placeholder="Nombre o RTN"></div>
                <div class="fa-field"><label>Etapa actual</label><select class="form-control" wire:model.live="filtroEtapa"><option value="">Todas</option>@foreach($catalogos['etapas'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Estado</label><select class="form-control" wire:model.live="filtroEstado"><option value="">Todos</option><option value="1">Activo</option><option value="7">Finalizado</option></select></div>
                <div class="fa-field"><label>Tipo de venta</label><select class="form-control" wire:model.live="filtroTipoVenta"><option value="">Todos</option>@foreach($catalogos['tipos_venta'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Asesor comercial</label><select class="form-control" wire:model.live="filtroAsesor"><option value="">Todos</option>@foreach($catalogos['usuarios'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Teleasesor / creador</label><select class="form-control" wire:model.live="filtroTeleasesor"><option value="">Todos</option>@foreach($catalogos['usuarios'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Usuario participante</label><select class="form-control" wire:model.live="filtroUsuario"><option value="">Todos</option>@foreach($catalogos['usuarios'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Oferta</label><input type="number" min="1" class="form-control" wire:model.live="filtroOferta" placeholder="ID oferta"></div>
                <div class="fa-field"><label>Prefactura</label><input type="number" min="1" class="form-control" wire:model.live="filtroPrefactura" placeholder="ID prefactura"></div>
                <div class="fa-field"><label>Factura</label><input type="number" min="1" class="form-control" wire:model.live="filtroFactura" placeholder="ID factura"></div>
                <div class="fa-field"><label>Producto</label><input type="text" class="form-control" wire:model.live="filtroProducto" placeholder="ID, código o nombre"></div>
                <div class="fa-field"><label>Marca</label><select class="form-control" wire:model.live="filtroMarca"><option value="">Todas</option>@foreach($catalogos['marcas'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Categoría</label><select class="form-control" wire:model.live="filtroCategoria"><option value="">Todas</option>@foreach($catalogos['categorias'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
                <div class="fa-field"><label>Equipo de entrega</label><select class="form-control" wire:model.live="filtroEquipo"><option value="">Todos</option>@foreach($catalogos['equipos'] as $item)<option value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>@endforeach</select></div>
            </div>
    ...
    <div class="modal fade" id="modalUmbralesFlujo" tabindex="-1" role="dialog" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document"><div class="modal-content">
            ...
            <div class="modal-body"><div class="fa-table-wrap"><table class="fa-table"><thead><tr><th>Etapa</th><th>Normal hasta (min)</th><th>Advertencia hasta (min)</th><th>Crítico</th></tr></thead><tbody>
                @foreach($umbralesEditables as $tipo => $limites)<tr><td><strong>{{ $limites['nombre'] }}</strong></td><td><input type="number" min="0" class="form-control form-control-sm" wire:model.live="umbralesEditables.{{ $tipo }}.normal"></td><td><input type="number" min="1" class="form-control form-control-sm" wire:model.live="umbralesEditables.{{ $tipo }}.advertencia"></td><td>&gt; {{ $limites['advertencia'] }} min</td></tr>@endforeach
            </tbody></table></div></div>
