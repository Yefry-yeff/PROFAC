                            ...
                            <div class="row">
                                <div class="col-12 col-md-6 col-lg-6 col-xl-6">
                                    ...
                                    <div class="input-group">
                                        <input type="text" id="codigoProductoFacturaCorp" class="form-control"
                                               placeholder="ID o nombre del producto…" autocomplete="off"
                                               onkeydown="if(event.key==='Enter'){buscarPorCodigoFacturaCorp(this.value);return false;}">
                                        <div class="input-group-append">
                                            <button type="button" class="btn btn-primary" title="Buscar producto"
                                                    onclick="limpiarProductoFacturaCorp(); window['abrirBuscador_buscadorProductoFacturaCorp'](document.getElementById('codigoProductoFacturaCorp').value||'')">
                                    ...
                                    @push('scripts')
                                    <script>
                                    function limpiarProductoFacturaCorp() {
                                        document.getElementById('seleccionarProducto').innerHTML = '<option value="" selected disabled></option>';
                                        document.getElementById('codigoProductoFacturaCorp').value = '';
                                        var lbl = document.getElementById('productoSeleccionadoFacturaCorp');
                                        lbl.classList.add('d-none'); lbl.textContent = '';
                                        document.getElementById('historialPreciosPanel').classList.add('d-none');
                                    }
                                    function alSeleccionarProductoFacturaCorp(producto) {
                                        ...
                                        document.getElementById('codigoProductoFacturaCorp').value = producto.nombre;
                                        var label = document.getElementById('productoSeleccionadoFacturaCorp');
                                        label.textContent = '✓ ' + producto.nombre + ' (ID: ' + producto.id + ')';
                                        label.classList.remove('d-none');
                                        obtenerImagenes();
                                        cargarHistorialPreciosFacturaCorp();
                                    }
                                    function buscarPorCodigoFacturaCorp(cod) {
                                        cod = String(cod).trim();
                                        if (!cod) { window['abrirBuscador_buscadorProductoFacturaCorp'](''); return; }
                                        axios.get('/productos/buscar', { params: { q: cod, page: 1 } })
                                            .then(function(r) {
                                                ...
                                                if (exact) { alSeleccionarProductoFacturaCorp(exact); }
                                                else if (items.length === 1) { alSeleccionarProductoFacturaCorp(items[0]); }
                                                else { window['abrirBuscador_buscadorProductoFacturaCorp'](cod); }
                                            });
                                    }
                                    function cargarHistorialPreciosFacturaCorp() {
                ...
                axios.post('/ventas/corporativo/guardar', formDataObj, options)
                    .then(response => {
                        ...
                        document.getElementById('seleccionarProducto').innerHTML =
                            '<option value="" selected disabled></option>';
                        document.getElementById('codigoProductoFacturaCorp').value = '';
                        var lbl_p = document.getElementById('productoSeleccionadoFacturaCorp');
